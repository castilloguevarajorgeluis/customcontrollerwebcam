sap.ui.define([
	...
	CControl = Control.extend("ynstestcamera.ywatestcamera.personalcontroller.Camera", {
		...
		init: function () {
			//This code was generated by the layout editor.
			goBusyDialog = new sap.m.BusyDialog();
		...
		_onSubmitTakePicture: function (oEvent) {
			...
			if (oVideo.videoWidth && oVideo.videoWidth !== 0 
				&& oVideo.videoWidth !== this.getVideoWidth()) {
				
				this.setVideoWidth(oVideo.videoWidth + "px");
			}
			if (oVideo.videoHeight && oVideo.videoHeight !== 0 
				&& oVideo.videoHeight !== this.getVideoHeight()) {
				
				this.setVideoHeight(oVideo.videoHeight + "px");
			}
			this._fnDeviceRescale();
		...
		_getVideo: function () {
			...
			var fplay = function () {
				...
				var fnStopInterval = function (iInternalValue, bStop) {
					iCont += 1;
					var iValue = iInternalValue * iCont;
					if (iValue > 10000 || bStop) {
						clearInterval(giInterval);
						gbBusy = false;
						goBusyDialog.close();
					}
				};
				var fnInterval = function () {
					...
					if ((!that.getVideoWidth() || that.getVideoWidth() === "0px" ) 
						...
						fnStopInterval(giInterval, true);
					}
					...
					fnStopInterval(giInterval, false);
				};
				giInterval = setInterval(fnInterval, 100);
			};
			if (oVideo) {
		...
		_fnDeviceRescale: function () {
			...
			var iVideoRelation = iVideoWidth / iVideoHeight;
			var bVideoLandscape = true;
			if (iVideoRelation < 1) {
				iVideoRelation = iVideoHeight / iVideoWidth;
				bVideoLandscape = false;
			}
			if (bVideoLandscape) {
				if (iVideoWidth > sap.ui.Device.resize.width) {
					iVideoWidth = sap.ui.Device.resize.width;
					iVideoHeight = parseInt((iVideoWidth / iVideoRelation), 10);
				}
			} else {
				if (iVideoHeight > sap.ui.Device.resize.height) {
					iVideoHeight = sap.ui.Device.resize.height;
					iVideoWidth  = parseInt((iVideoHeight / iVideoRelation), 10);
				}
			}
			this.setVideoWidth(iVideoWidth + "px");
			this.setVideoHeight(iVideoHeight + "px");
		},
		...
		_onStopCamera: function () {
			...
			if (this.oMediaRecorder) {
				if (this.oMediaRecorder.state) {
					if (this.oMediaRecorder.state !== "inactive") {
						this._onSubmitStopRecord();
					}
				}
		...
		renderer: function (oRm, oControl) {
			...
			goBusyDialog.open();
			while (gbBusy === true) {
				if (oControl.getVideoWidth() || oControl.getVideoHeight()) {
					gbBusy = false;
					clearInterval(giInterval);
				}
			}
